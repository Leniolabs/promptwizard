...
class functionCalling:
    def __init__(self, description, test_cases, number_of_prompts, model_test, model_test_temperature, model_test_max_tokens, model_generation, model_generation_temperature, prompts, functions, function_call):
...
Most importantly, output NOTHING but the prompt. Do not include anything else in your message."""
    ...
    def test_candidate_prompts(self):
        ...
        results = [{"description": self.description, "method": "Function_calling"}]
        for prompt in self.prompts:
            ...
            for test_case in self.test_cases:
                ...
                # Update model results
                    ...
                    if test_case[1] == response.choices[0].message.function_call.name and json_object['file_type'] == test_case[2]:
                        prompt_results[prompt]['correct'] += 1
                    prompt_results[prompt]['total'] += 1
                    if test_case[1] == response.choices[0].message.function_call.name and test_case[2] == json_object['file_type']:
                        a = True
                    if test_case[1] == response.choices[0].message.function_call.name and not (test_case[2] == json_object['file_type']):
                        a = 'variable error'
                    if not (test_case[1] == response.choices[0].message.function_call.name) and test_case[2] == json_object['file_type']:
                        a = 'function error'
                    if not (test_case[1] == response.choices[0].message.function_call.name) and (not test_case[2] == json_object['file_type']):
                        a = 'function and variable error'
                    prompt_and_results.append({"test": test_case[0], "answer": {"function": f"{response.choices[0].message.function_call.name}", "variable": f"{json_object['file_type']}"}, "ideal": {"function": f"{test_case[1]}", "variable": f"{test_case[2]}"}, "result": a})
                ...
                if "function_call" not in response['choices'][0]['message']:
                    prompt_and_results.append({"test": test_case[0], "answer": 'not a function call', "ideal": f"{test_case[1]}" + " " f"{test_case[2]}", "result": 'Received text data instead of JSON.'})
                    prompt_results[prompt]['total'] += 1
